...
# Adapted for the OSGi bug tracker migration by BJ Hargrave
...
#    - Run a report
#    - at the bottom of the report page click the XML icon
#    - save the XML into a file: bugzilla.xml
# 3. Run the migration script and check all the warnings:
#    bugzilla2github -x bugzilla.xml -o osgi -r osgi -t githubPAT
# 4. Run the migration script again and force the updates:
#    bugzilla2github -x bugzill.xml -o osgi -r osgi -t githubPAT -f
...
email2login = {
    ...
    "__default__": None,
    "raymond.auge@liferay.com": "rotty3000",
    "bosschae@adobe.com": "bosschaert",
    "davidb@iona.com": "bosschaert",
    "hargrave@us.ibm.com": "bjhargrave",
    "tjwatson@us.ibm.com": "tjwatson",
    "tim.ward@paremus.com": "timothyjward",
    "timothy.ward@uk.ibm.com": "timothyjward",
    "cziegele@adobe.com": "cziegeler",
    "tim.verbelen@ugent.be": "tverbele",
    "tim.verbelen@intec.ugent.be": "tverbele",
    "todor.boev@softwareag.com": "rinswind",
    "j.albert@data-in-motion.biz": "juergen-albert",
    "m.hoffmann@data-in-motion.biz": "maho7791",
    "maeomichi.hiroyuki@lab.ntt.co.jp": "maeomichi",
    "anjum.fatima@ibm.com": "anjumfatima90",
    "jjh@aicas.de": "jjh-aicas",
    "peter.kriens@aqute.biz": "pkriens",
}
status2state = {
...
def attachment_convert(idx, attach):
    ...
    ret.append("> Attached file: [%s](https://osgi.org/members/bugzilla/attachment.cgi?id=%s) (%s, %s bytes)" % (attach.pop("filename"), id, attach.pop("type"), attach.pop("size")))
    if "desc" in attach:
...
def date_convert(date):
    ...
    if not result:
        ...
        exit(1)
    return "{a}T{b}{c}{d}:{e}".format(a = result.group(1), b = result.group(2),
                                    c = result.group(3), d = result.group(4), e = result.group(5))
...
def bug_convert(bug):
    ...
    ret["body"] = []
    #ret["body"].append("Note: the issue was created automatically migrated from https://osgi.org/members/bugzilla/show_bug.cgi?id=%d" % ret["number"])
    #ret["body"].append("")
    ret["labels"] = []
    ret["comments"] = []
    ...
    ret["body"].append("Original bug ID: [BZ#%d](https://osgi.org/members/bugzilla/show_bug.cgi?id=%d)" % (ret["number"], ret["number"]))
